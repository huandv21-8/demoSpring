<%@ page language="java" pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<title>Tutorial: Hello Dojo!</title>
<link rel="stylesheet"
	href='<c:url  value="/resources/css/slicknav.min.css" />' />
<link rel="stylesheet"
	href='<c:url  value="/resources/css/owl.carousel.min.css" />' />
<link rel="stylesheet"
	href='<c:url  value="/resources/css/magnific-popup.css" />' />
<link rel="stylesheet"
	href='<c:url  value="/resources/css/jquery-ui.min.css" />' />
<link rel="stylesheet"
	href='<c:url  value="/resources/css/font-awesome.min.css" />' />
<link rel="stylesheet"
	href='<c:url  value="/resources/css/elegant-icons.css" />' />
<link rel="stylesheet"
	href='<c:url  value="/resources/css/bootstrap.min.css" />' />
<!-- load Dojo -->

</head>
<body>

	<div class="container">
		<h2>Horizontal form</h2>
		${check }
		<form class="form-horizontal" method="post"
			action='<c:url value="/edit" />'>
			<div class="form-group">
				<label class="control-label col-sm-2" for="email">ID
					product:</label>
				<div class="col-sm-10">
					<input type="text" class="form-control" id="id"
						placeholder="Enter name product" value="${product.id_product }"
						readonly name="id">
				</div>
			</div>
			<div class="form-group">
				<label class="control-label col-sm-2" for="email">Name
					product:</label>
				<div class="col-sm-10">
					<input type="text" class="form-control" id="nameProduct"
						placeholder="Enter name product" value="${product.nameProduct }"
						name="nameProduct">
				</div>
			</div>
			<div class="form-group">
				<label class="control-label col-sm-2" for="pwd">Description:</label>
				<div class="col-sm-10">
					<input type="text" value="${product.description }"
						class="form-control" id="description"
						placeholder="Enter description" name="description">
				</div>
			</div>
			<div class="form-group">
				<label class="control-label col-sm-2" for="pwd">Price:</label>
				<div class="col-sm-10">
					<input type="number" value="${product.price }" class="form-control"
						id="price" placeholder="Enter price" name="price">
				</div>
			</div>
			<div class="form-group">
				<label class="control-label col-sm-2" for="pwd">Image:</label>
				<div class="col-sm-10">
					<input type="text" value="${product.image }" class="form-control"
						id="image" placeholder="Enter image" name="image">
				</div>
			</div>

			<div class="form-group">
				<div class="col-sm-offset-2 col-sm-10">
					<button type="submit" class="btn btn-success">Submit</button>
				</div>
			</div>
		</form>
	</div>

</body>
</html>